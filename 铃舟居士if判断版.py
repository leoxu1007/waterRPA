...
def screenFind(waitingTime, img):
    consumeTime = 0
    while consumeTime <= waitingTime:
...
def mainWork(img):
    ...
    while i < sheet1.nrows:
        ...
        cmdType = sheet1.row(i)[0]
        # if判断条件匹配不到 跳过从句
        if cmdType.value == 10.0 and warikomi == 2:
            warikomi = 0
            print("if从句已经全部跳过")
            i += 1
            continue
